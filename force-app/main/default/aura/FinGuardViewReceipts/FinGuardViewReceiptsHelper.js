...
({
    init : function(component) {
        ...
        let action = component.get("c.getReceiptsList");
        action.setCallback(this,function(response){
            ...
            if(state === "SUCCESS"){
                ...
                component.set("v.receiptsList", response.getReturnValue());
                let displayedList = component.get("v.receiptsList");
    ...
    paginate : function(component) {
        console.log("In paginate helper");
    ...
    toNextPage : function(component) {
        let desplayedList = component.get("v.displayedList");
        ...
        for(let i=0; i<pageLength; i++){
            to++;
            if(desplayedList.length > to){
                pageList.push(desplayedList[to]);
            }
            from++;
        }
        component.set("v.pageList",pageList);
    ...
    filterByDate: function(component){
        ...
        if(date.length) {
            ...
            let displayedList = component.get("v.displayedList");
            for (let i = 0; i < displayedList.length; i++) {
                if (displayedList[i].Receipt_Date__c === date) {
                    searchList.push(displayedList[i]);
                }
            }
            component.set("v.displayedList", searchList);
    ...
    resetFilter: function(component) {
        let resetDisplayedList = component.get("v.receiptsList");
        component.set("v.displayedList", resetDisplayedList);
        component.set("v.from",0);
        component.set("v.to",component.get("v.pageLength")-1);
        this.paginate(component);
    },
    changeReceipt:function (component,event,checkedReceipt){
        let appEvent = $A.get("e.c:ToChangeReceiptEvent");
        appEvent.setParams({
